...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: topupEnabledSetting } = await supabaseAdmin
            .from('admin_settings')
            .select('value')
            .eq('key', 'wallet_topup_enabled')
            .single()

        if (topupEnabledSetting?.value === 'false') {
            return NextResponse.json(
                { error: 'Wallet top-up is temporarily unavailable. Please try again later.' },
                { status: 403 }
            )
        }
        ...
        console.log('[WalletTopup] Creating payment record:', { userId, walletId: (wallet as any)?.id, amount, fee, totalAmount, reference })
        ...
        if (paymentError) {
            console.error('[WalletTopup] Payment record insert error:', JSON.stringify(paymentError))
            return NextResponse.json({
        ...
        const paystackResponse = await fetch('https://api.paystack.co/transaction/initialize', {
            ...
            body: JSON.stringify({
                ...
                // metadata: { ... } // Removed to prevent "Authorization code not found" error
            }),
        })
