...
export default function UpgradePage() {
    ...
    const { startTutorial } = useTutorial(userRole as 'user' | 'agent', '/upgrade')
    ...
    if (isLoading || isVerifying) {
        return (
            <div className="fixed inset-0 bg-[#FFCE00] dark:bg-yellow-600 flex items-center justify-center z-50 p-6">
                <div className="flex flex-col items-center gap-6 max-w-sm w-full">
                    <div className="relative">
                        <Crown className="w-16 h-16 text-yellow-800 dark:text-yellow-100 animate-bounce relative z-10" />
                    </div>
                    {isVerifying && (
                        <div className="text-center space-y-4 w-full">
                            <div className="space-y-2">
                                <h2 className="text-2xl font-black text-yellow-900 dark:text-yellow-50">Verifying Payment...</h2>
                                <p className="text-yellow-800 dark:text-yellow-200 font-bold animate-pulse">Please wait while we activate your agent status.</p>
                            </div>
    ...
    return (
        <>
            ...
            <div className="relative -m-4 sm:-m-6 min-h-[calc(100vh+2rem)] lg:min-h-screen bg-gradient-to-br from-yellow-400 via-amber-500 to-yellow-600 dark:from-yellow-700 dark:via-amber-800 dark:to-yellow-900 overflow-x-hidden selection:bg-yellow-200 px-4 sm:px-6 py-10 sm:py-16 flex flex-col items-center scroll-smooth" style={{ fontFamily: '"Fira Sans", sans-serif' }}>
                ...
                <div className="relative z-10 w-full max-w-6xl flex flex-col items-center">
                    ...
                    <div className="text-center mb-8 sm:mb-12 space-y-3 sm:space-y-4 relative w-full">
                        ...
                        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-yellow-900 dark:text-yellow-500 leading-tight">
                            {dbUser?.role === 'agent' ? (
                        ...
                        {dbUser?.role === 'agent' && (
                            <div className="inline-block px-4 py-1.5 rounded-full bg-white/20 dark:bg-yellow-100/10 border border-white/30 dark:border-yellow-200/20 backdrop-blur-sm shadow-sm group">
                                <span className="text-[10px] sm:text-xs font-black text-yellow-900 dark:text-yellow-500 uppercase tracking-[0.2em] flex items-center gap-2">
                                    <Crown className="w-3 h-3 fill-yellow-900 dark:fill-yellow-500 text-yellow-900 dark:text-yellow-500 group-hover:rotate-180 transition-transform duration-700" />
                                    PREMIUM MEMBERSHIP
                                    <Crown className="w-3 h-3 fill-yellow-900 dark:fill-yellow-500 text-yellow-900 dark:text-yellow-500 group-hover:rotate-180 transition-transform duration-700" />
                                </span>
                            </div>
                        ...
                        <p id="agent-benefits" className="text-sm sm:text-base lg:text-lg text-yellow-950 dark:text-white font-black max-w-3xl mx-auto px-4 drop-shadow-sm">
                            {dbUser?.role === 'agent'
                    ...
                    <div id="pricing-plans" className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6 mb-12 w-full max-w-5xl items-stretch">
                        {tiers.map((tier) => (
                            <div
                                key={tier.id}
                                className={cn(
                                    "relative rounded-2xl p-6 flex flex-col transition-all duration-500 shadow-xl border-2 backdrop-blur-md",
                                    tier.bgClass,
                                    "dark:bg-black/80",
                                    tier.color,
                            ...
                            >
                                ...
                                <div className="text-center mb-5 space-y-1">
                                    <h3 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white border-b-2 border-yellow-100/50 dark:border-yellow-500/20 pb-2">{tier.name}</h3>
                                    <p className="text-gray-500 dark:text-slate-400 text-xs sm:text-sm font-bold pt-1">{tier.duration}</p>
                                </div>
                                    ...
                                    {commonFeatures.map((feature, i) => {
                                        ...
                                        return (
                                            <div key={i} className="flex items-start gap-2.5">
                                                ...
                                                <span className="text-xs sm:text-sm font-bold text-gray-700 dark:text-slate-200 leading-snug">{feature}</span>
                                            </div>
                                        )
                                    })}
                                </div>
                    ...
                    <div className="w-full max-w-3xl">
                        <div className="bg-white/60 dark:bg-black/60 backdrop-blur-xl rounded-2xl p-6 sm:p-10 border-2 border-white/20 dark:border-white/5 shadow-xl flex flex-col items-center text-center space-y-6">
                            <div className="inline-flex items-center gap-2 text-amber-700 dark:text-amber-500 bg-yellow-100/50 dark:bg-yellow-500/10 px-4 py-1.5 rounded-full border border-yellow-200/50 dark:border-yellow-500/20">
                                <Zap className="w-5 h-5 fill-current" />
                                <span className="text-xs font-black uppercase tracking-[0.2em]">FAST & SECURE</span>
                            </div>
                            <p className="text-base text-gray-600 dark:text-slate-300 font-bold leading-relaxed max-w-xl">
                                Upgrade your account in seconds using card, mobile money or bank transfer via Paystack. Your benefits start immediately.
                            </p>
                            <div className="flex items-center gap-3 pt-2 grayscale opacity-50 dark:opacity-30">
                                <div className="w-7 h-7 bg-gray-200 dark:bg-slate-800 rounded-lg flex items-center justify-center text-[10px] font-black text-gray-600 dark:text-slate-400 border border-gray-300 dark:border-slate-700">?</div>
                                <span className="text-[11px] font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest">SECURE PAYMENTS BY PAYSTACK</span>
                            </div>
                        </div>
                    </div>
                </div>
