...
export default function AdminOrdersPage() {
    ...
    const handleExportExcel = async () => {
        ...
        const pendingOrders = (Array.isArray(filteredOrders) ? filteredOrders : [])
    ...
    function BatchCard({ batch, onRefund, onFail }: { batch: any, onRefund?: any, onFail?: any }) {
        ...
        const loadBatchOrders = async () => {
            if (hasLoadedOnce && batchOrders.length > 0) return
        ...
        return (
            <Card className={cn(
                "group relative overflow-hidden border-muted/40 shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col bg-gradient-to-br from-white to-slate-50 dark:from-slate-950 dark:to-slate-900/50 dark:border-blue-900/20",
                isExpanded ? "h-auto min-h-[400px] max-h-[800px]" : "h-fit min-h-0"
            )}>
                ...
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 flex-shrink-0 cursor-pointer select-none" onClick={toggleExpand}>
                    <div className="flex flex-col overflow-hidden">
                        <CardTitle className="text-sm font-black truncate max-w-[200px]" title={batch.filename}>
                            {batch.filename}
                        </CardTitle>
                    ...
                    <div className="flex items-center gap-2">
                        <div className="text-right mr-2 hidden md:block">
                            <div className="text-xs font-bold text-blue-600">{batch.order_count} orders</div>
                            <div className="text-[10px] text-muted-foreground">{formatDate(batch.created_at)}</div>
                        </div>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-muted/50">
                            {isExpanded ? <ChevronDown className="w-4 h-4 rotate-180 transition-transform duration-300" /> : <ChevronDown className="w-4 h-4 transition-transform duration-300" />}
                        </Button>
                    </div>
                </CardHeader>

                {!isExpanded && batch.status_summary && (
                    <div className="px-4 pb-3 flex items-center gap-2 -mt-1 animate-in fade-in slide-in-from-top-1 duration-300">
                        {(() => {
                            const { pending, processing, completed, failed } = batch.status_summary
                            if (processing > 0) return <Badge className="bg-blue-100 text-blue-700 border-blue-200 text-[10px] py-0 px-1.5 h-5">Processing ({processing}/{batch.order_count})</Badge>
                            if (pending > 0) return <Badge className="bg-amber-100 text-amber-700 border-amber-200 text-[10px] py-0 px-1.5 h-5">Pending ({pending}/{batch.order_count})</Badge>
                            if (completed === batch.order_count) return <Badge className="bg-green-100 text-green-700 border-green-200 text-[10px] py-0 px-1.5 h-5">Completed</Badge>
                            if (failed === batch.order_count) return <Badge className="bg-red-100 text-red-700 border-red-200 text-[10px] py-0 px-1.5 h-5">Failed</Badge>
                            return <Badge variant="outline" className="text-[10px] py-0 px-1.5 h-5">Mixed Status</Badge>
                        })()}
                        {batch.status_summary.completed > 0 && batch.status_summary.completed < batch.order_count && (
                             <span className="text-[10px] text-muted-foreground font-medium">{batch.status_summary.completed} done</span>
                        )}
                    </div>
                )}

                <CardContent className={cn(
                    "flex-1 overflow-hidden flex flex-col py-2 transition-all duration-300",
                    isExpanded ? "opacity-100 max-h-[1000px]" : "opacity-0 max-h-0 py-0 pointer-events-none"
                )}>
                    <div className="flex items-center justify-between mb-2 flex-shrink-0 md:hidden">
                    ...
                    <div className="flex-1 overflow-y-auto border rounded-xl bg-muted/5 p-2 space-y-2 min-h-[200px] max-h-[400px]">
                        {isLoadingOrders ? (
                            <div className="h-full flex flex-col items-center justify-center py-8">
                                <Loader2 className="w-6 h-6 animate-spin text-blue-500 mb-2" />
                                <span className="text-xs text-muted-foreground font-bold">Loading orders...</span>
                            </div>
                        ) : batchOrders.length === 0 ? (
                            <div className="h-full flex items-center justify-center text-xs text-muted-foreground py-8">
                                No orders found in this batch
                            </div>
                        ) : (
                            Array.isArray(batchOrders) && batchOrders.map(order => (
                                <div key={order.id} className="text-xs p-2 bg-background/50 backdrop-blur-sm rounded-lg border flex items-center justify-between hover:border-blue-500/30 transition-colors">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-tight">{order.network} {order.size}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {order.payment_status === 'refunded' ? (
                                            ...
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="ghost" size="icon" className="h-7 w-7 p-0 hover:bg-muted rounded-full">
                                                        <MoreVertical className="w-3.5 h-3.5" />
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent align="end" className="rounded-xl border-muted/40 shadow-xl">
                ...
                {isExpanded && (
                    <CardFooter className="p-4 pt-2 border-t flex-shrink-0 gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <Button
                            className="flex-1 bg-green-600 hover:bg-green-700 text-white font-black rounded-xl"
                            size="sm"
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="icon" className="h-9 w-9 bg-muted/30 border-muted/50 rounded-xl">
                                    <MoreVertical className="w-4 h-4" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="rounded-xl border-muted/40 shadow-xl">
                                <DropdownMenuItem onClick={() => onUpdateBatchStatus('processing')} className="rounded-lg">
                                    <Clock className="w-4 h-4 mr-2 text-blue-500" />
                                    Mark All Processing
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => onUpdateBatchStatus('failed')} className="rounded-lg">
                                    <XCircle className="w-4 h-4 mr-2 text-red-500" />
                                    Mark All Failed
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => reDownloadBatch(batch)} className="rounded-lg">
        ...
        <div className="space-y-6">
            <div className="flex flex-col items-center gap-4 text-center">
                <div>
                    ...
                    <p className="text-muted-foreground">View and manage all user orders</p>
                </div>
