...
export function useTutorial(userRole: UserRole, currentPage: PagePath): UseTutorialReturn {
    ...
    const getStepsForPage = useCallback((): DriveStep[] => {
        const { whatsappGroupLink: group, whatsappChannelLink: channel } = settings;
        switch (currentPage) {
            case '/dashboard':
                return getDashboardTutorialSteps(userRole, group, channel);
            case '/orders':
                return getOrderHistoryTutorialSteps(group, channel);
            case '/complaints':
                return getComplaintsTutorialSteps(group, channel);
            case '/profile':
                return getProfileTutorialSteps(group, channel);
            case '/upgrade':
                return getUpgradeTutorialSteps(group, channel);
            case '/wallet':
                return getWalletTutorialSteps(userRole, group, channel);
            case '/data-packages':
                return getDataPackagesTutorialSteps(userRole, group, channel);
            default:
                // Default to dashboard steps if page not recognized
                return getDashboardTutorialSteps(userRole, group, channel);
        }
    }, [currentPage, userRole, settings]);
